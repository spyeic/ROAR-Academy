...
while True:
    ...
    hour_vector = clock_hand_vector((hour * 3600 + minute * 60 + second)/43200*2*np.pi, hour_hand_length)
    minute_vector = clock_hand_vector((minute * 60 + second)/3600*2*np.pi, minute_hand_length)
    second_vector = clock_hand_vector(second/60*2*np.pi, second_hand_length)
    utc_vector = clock_hand_vector((utc_time.hour * 3600 + utc_time.minute * 60 + utc_time.second)/86400*2*np.pi, hour_hand_length)
    ...
    plt.arrow(center[0], center[1], utc_vector[0], utc_vector[1], head_length = 3, linewidth = hour_hand_width, color = 'yellow')
